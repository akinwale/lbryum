...
def init_gui(config, network, daemon, plugins):
    ...
    if gui_name in ['lite', 'classic']:
        gui_name = 'qt'
    gui = __import__('lbryum_gui.' + gui_name, fromlist=['lbryum_gui'])
...
def init_cmdline(config_options):
    ...
    if cmd.requires_wallet and not storage.file_exists:
        ...
        print_msg("Type 'lbryum create' to create a new wallet, or provide a path to a wallet with the -w option")
        sys.exit(0)
